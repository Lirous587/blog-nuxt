<template>
  <div class="relative h-full ml-2 mt-3">
    <div
      class="h-[calc(100%-40px)] min-w-[800px] overflow-scroll"
      v-loading="loading"
    >
      <div class="grid grid-cols-4 gap-2">
        <div
          v-for="(item, index) in tableData"
          class="relative h-[180px] shadow-lg border border-gray-200 overflow-hidden rounded-lg dark:border-gray-700"
          :key="item.id"
        >
          <el-image
            :src="imgPre + item.url"
          ...
          >
            <div
              ...
              size="small"
            >
              <span class="ml-2 text-red-950 line-clamp-2">
                {{ item.url }}</span
              >
            </div>
            <!-- 选择框 -->
    ...
    <div class="flex justify-center absolute left-0 right-0 bottom-[45px]">
      <el-pagination
        background
    ...
    <div class="absolute right-5 bottom-[45px]" v-if="ifSelect">
